...
export default function GMPage() {
    ...
    useEffect(() => {
        if (!user || user.role !== 'KEEPER' || selectedSessionId === 'ALL') return;
    ...
    if (isLoading || !user || user.role !== 'KEEPER' || isLoadingData) return <div className="p-8 text-[var(--color-mythos-parchment)] animate-pulse">Consultando Arquivos Confidenciais...</div>;
    ...
    return (
        <div className="h-[calc(100vh-8rem)] flex flex-col">
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-hidden">
                ...
                <div className={`lg:col-span-4 overflow-y-auto pr-2 space-y-4 ${selectedId ? 'hidden lg:block' : 'col-span-12'}`}>
                    {investigators.length === 0 && !isLoadingData && (
                ...
                {/* Right Column: Sheet Display (8 cols) */}
                {selectedId && investigator ? (
                    <div className="lg:col-span-8 h-full overflow-hidden border border-[var(--color-mythos-gold-dim)]/30 rounded-lg bg-[var(--color-mythos-black)]/50 relative flex flex-col animate-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="absolute top-2 right-4 z-[60] flex items-center gap-2">
                            ...
                            <Button size="icon" variant="ghost" className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-900/20 lg:hidden" onClick={handleClose}>
                                <X className="w-4 h-4" />
                            </Button>
                        </div>
                    ...
                    <div className="hidden lg:flex lg:col-span-8 h-full items-center justify-center border-2 border-dashed border-[var(--color-mythos-gold-dim)]/20 rounded-lg bg-[var(--color-mythos-black)]/20 text-[var(--color-mythos-gold-dim)]/50">
                        <div className="text-center">
                            <Eye className="w-16 h-16 mx-auto mb-4 opacity-20" />
                            <p className="text-xl uppercase tracking-widest font-heading">Select an Investigator</p>
                            <p className="text-sm font-serif italic">to inspect their detailed records</p>
                        </div>
                    </div>
                )}
            </div>
